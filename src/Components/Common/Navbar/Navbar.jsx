...
import { assets, services } from '../../../Datasets/services'
            ...
            <div className={`nav-links-div ${mobileMenu ? "menu-open" : ""}`}>
                <ul className={`menu-bar ${mobileMenu ? "mobile-menu-open" : "hide-mobile-menu"}`}>
                    ...
                    <li 
                        ...
                        <a href="/assets" className={location.pathname==="/assets" ? "" : ""}>Portfolio</a>
                        <span 
                            className="dropdown-toggle"
                            style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center', zIndex: 10, position: 'relative' }}>
                            <IoIosArrowDown />
                        ...
                        {/* Desktop dropdown - shown on hover via CSS */}
                        <div className="dropdown asssets desktop-dropdown">
                            <ul>
                                {assets.map((ele) => {
                                    return (
                                        <a href={`/${ele.slug}`} className={location.pathname === `${ele.slug}` ? "active" : ""}><li key={ele.id || ele.title}>{ele.title}</li></a>
                                    )
                                })}
                            </ul>
                        </div>
                            ...
                            {isServicesDropDownOpen && (
                                <motion.div 
                                    ...
                                    <motion.ul
                                        ...
                                        {assets.map((ele) => {
                                            return (
                                                <li key={ele.id || ele.title}><a href={`/${ele.slug}`} className={location.pathname === `/${ele.slug}` ? "active" : ""}>{ele.title}</a></li>
                                            )
                                        })}
                                    </motion.ul>
                                </motion.div>
